...
> **Priority:** Strip containers off `iamfaulty-mini` (M4, 16GB) so OrbStack can quit.
...
|------|-----------|--------------------|
| **iamfaulty-mini** | M4 Mac, 16GB, arm64 | **ONLY** iMessage bridge, OpenClaw, inference worker. OrbStack off. |
| **minifw** | `192.168.68.64`, Ubuntu 26.04, 15GB, x86_64 | Arr stack + migrated apps (incl. homepage, eventually caddy/sync) |
| **ILLMATIC** | `192.168.68.69` NAS | Compose at `/volume3/homelab/compose/`. **Cannot** host containers. |
| **Pi 5** | 8GB, aarch64 | Edge cluster: blocky, homeassistant, netalertx, speedtest-tracker, uptime-kuma |

NFS on minifw: `/mnt/homelab` → `192.168.68.69:/volume3/homelab` (fstab).

## Two bugs that bite EVERY Mac → minifw move
...
| → minifw | jellyfin, shelfarr, audiobookshelf, sonobarr, duplicati, gitea, anythingllm, dozzle, beszel, planka |
...
|---|------|----------|-------|---------|
| 1 | **homepage → minifw** | small | Apply UID/platform fixes | [`homepage/MIGRATE.md`](homepage/MIGRATE.md) |
| 2 | **Edge → Pi 5** (non-DNS first) | ~363MB total | homeassistant, netalertx, speedtest-tracker, uptime-kuma | [`edge-pi5/MIGRATE.md`](edge-pi5/MIGRATE.md) |
| 3 | **blocky → Pi 5** | (in edge) | **LAST** — LAN DNS. Flip DHCP/clients only after healthy dig. | same |
...
| Target compose (edge) | `reference/target-compose/edge-pi5/` |
| Target compose (proxy after split) | `reference/target-compose/minifw-proxy/` |
