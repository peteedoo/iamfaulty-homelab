...
Dashboard still on OrbStack. Move to minifw with the rest of the apps stack so the mini can shed another container before OrbStack quit.
...
| ports | `127.0.0.1:3005:3000` | keep if Caddy/NPM on same host; else bind as needed for minifw proxy |
...
# 2) On minifw: ensure compose yaml is the target (UID 1000), then:
...
- If **Caddy** still on mini and proxies `homepage.iamfaulty.com` → `host.docker.internal:3005`, update that route to **minifw** (`192.168.68.64:3005`) **before** or **as** homepage moves.
- Homepage widgets that scrape docker.sock only see **local** containers on minifw — remote hosts need Agent / API widgets (Beszel/Dozzle already on minifw help).

## Verify

- [ ] `docker compose ps` healthy on minifw
- [ ] UI loads on `:3005`
